...
author_directories = [
    f for f in os.listdir(directory_name) if os.path.isdir(directory_name + "/" + f)
]
for aut_dir in author_directories:
    ...
    for file in files:
        ...
        if name_parts[-1] == "txt":
            sentence_names.append(name_parts[0])
            ...
            for i in range(len(grid_lines) - grid_line_index):
                ...
                if len(tokens) >= 3:
                    ...
                    if (tokens[1] == "=") & (tokens[2][1:-1] == word):
                        time_start = float(
                            grid_lines[i + grid_line_index - 2].split(" ")[2]
                        )
                        time_end = float(
                            grid_lines[i + grid_line_index - 1].split(" ")[2]
                        )
                        print(
                            "  time: start = "
                            + str(time_start)
                        ...
                        if if_chart_in_the_time_domain:
                            ...
                            plt.title(
                                aut_path + "/" + sentence_name + "   word: " + word
                            )
                            plt.legend()
                            plt.xlabel("Time ")
