    ...
    for i, tg in enumerate(tg_paths):
        ...
        try:
            ...
            if np.issubdtype(audio_orig.dtype, np.integer):
                audio_orig = (
                    audio_orig.astype(np.float32) / np.iinfo(audio_orig.dtype).max
                )
            else:
            ...
            stacked = torch.stack(
                [
                    torch.stack(
                        file_windows_per_aug[a]
                    )  # (n_windows, n_mels, win_frames)
...
def build_augmented_cache_soft(
    data_dir,
    output_path,
    n_augmentations=5,
    standardize=True,
    ...
    for i, tg in enumerate(tg_paths):
        ...
        try:
            ...
            stacked = torch.stack(
                [torch.stack(file_windows_per_aug[a]) for a in range(n_augmentations)],
                dim=1,
            )
    ...
    print(f"final: X={X.shape}, y={y.shape}")
    print(
        f"rozmiar: {(X.element_size() * X.nelement() + y.element_size() * y.nelement()) / 1e9:.2f} GB"
    )
